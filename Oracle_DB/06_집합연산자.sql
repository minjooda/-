...
-- 데이터를 세로로 합쳐져 하나의 테이블처럼 나옴.
select '최대값' as Label, max(salary) as 집계결과 from emp 
union all select '최소값', min(salary) from emp
union all select '평균값', round(avg(salary), 2) from emp
order by 2; --집합연산을 끝낸 결과에 대한 정렬.

select * from emp where dept_id in (10, 20)
union all  --합집합: 중복된 것도 모두 나온다.
select * from emp where dept_id in (30, 20);
...
select * from emp where dept_id in (10, 20)
minus  --차집합: 첫번째 조회결과에 두번째 결과에 없는 것만 조회.
select * from emp where dept_id in (30, 20);
...
union select '급여합계', sum(salary) from emp
...
intersect select item from import_rank where year = 2018;
...
select '수출' as 품목, item from export_rank where year = 1990
union all
